        ...
        <motion.div
          ...
          <div className="absolute inset-0 pointer-events-none">
            {t.hero.floatingCards.map((card, i) => {
              // Distribute cards evenly around the orb (deterministic so cards
              // don't jump when toggling theme or language)
              const angle = i / t.hero.floatingCards.length * Math.PI * 2;
              const radiusX = 44; // percentage
              const radiusY = 40;
              ...
              return (
                <motion.div
                  ...
                  className="absolute glass px-4 py-2 rounded-xl text-xs md:text-sm font-medium text-slate-700 dark:text-slate-200 shadow-soft animate-float-slow whitespace-nowrap">
